...
export interface Database {
  ...
  updatePatientImageTimestamp(
    patientId: string,
    newTimestamp: string,
  ): Promise<void>;
  getLatestPatientEventByType(
    patient_id: string,
    event_type: string,
  ): Promise<string>;
  getAllPatientEventsByType(
    patient_id: string,
    event_type: string,
  ): Promise<Event[]>;
  ...
  // getVisitsInDateRange(startDate: Date, endDate: Date): Promise<Visit[]>;
  getPatientsVisitedInDateRange(startDate: Date, endDate: Date): Promise<any[]>;
}
...
class DatabaseImpl implements Database {
  ...
  public async saveStringContent(
    stringContent: StringContent[],
    id?: string,
  ): Promise<string> {
    ...
    if (!idExists) {
      await db.executeSql(`INSERT INTO string_ids (id) VALUES (?);`, [
        stringId,
      ]);
    }
  ...
  public async editStringContent(
    stringContent: StringContent[],
    id: string,
  ): Promise<string> {
    stringContent.forEach(async (element) => {
  ...
  private async editStringWithId(
    stringContent: StringContent,
    id: string,
  ): Promise<string> {
    ...
    const db = await this.getDatabase();
    await db.executeSql(
      `UPDATE string_content SET content = ?, edited_at = ? WHERE id = ? AND language = ?`,
      [stringContent.content, date, id, stringContent.language],
    );
    return id;
  }
  ...
  private async saveStringWithId(
    stringContent: StringContent,
    id: string,
  ): Promise<string> {
    const date = new Date().toISOString();
    ...
    await db.executeSql(
      ...
      [id, stringContent.language, stringContent.content, date],
    );
    return id;
  }
  ...
  public async addUser(user: NewUser, password: string): Promise<void> {
    ...
    await db.executeSql(
      `INSERT INTO users (id, name, role, email, hashed_password, instance_url, edited_at) VALUES (?, ?, ?, ?, ?, ?, ?);`,
      [
        id,
        user.name,
        user.role,
  ...
  public addPatient(patient: NewPatient): Promise<void> {
    ...
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          `INSERT INTO patients (id, given_name, surname, date_of_birth, country, hometown, section, serial_number, registered_by_provider_id, phone, sex, image_timestamp, edited_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`,
          [
            id,
            patient.given_name,
            patient.surname,
      ...
      .then(([results]) => {
        console.log(`[db] Added patient with id: "${id}"!`);
      });
  }
  ...
  public editPatient(patient: NewPatient): Promise<Patient> {
    ...
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          `UPDATE patients SET given_name = ?, surname = ?, date_of_birth = ?, country = ?, hometown = ?, section = ?, registered_by_provider_id = ?, serial_number = ?, phone = ?, sex = ?, image_timestamp = ?, edited_at = ? WHERE id = ?`,
          [
            patient.given_name,
            patient.surname,
            patient.date_of_birth,
  ...
  public editEvent(id: string, event_metadata: string): Promise<Event[]> {
    ...
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          `UPDATE events SET event_metadata = ?, edited_at = ? WHERE id = ?`,
          [event_metadata, date, id],
        ),
      )
      .then(async ([results]) => {
  ...
  public editVisitDate(visit_id: string, date: string): Promise<void> {
    ...
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          `UPDATE visits SET check_in_timestamp = ?, edited_at = ? WHERE id = ?`,
          [date, edited_at, visit_id],
        ),
      )
      .then(() => {
        return;
  ...
  public updatePatientImageTimestamp(
    patientId: string,
    newTimestamp: string,
  ): Promise<void> {
    const date = new Date().toISOString();
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          `UPDATE patients SET image_timestamp = ?, edited_at = ? WHERE id = ?`,
          [newTimestamp, date, patientId],
        ),
      )
      .then(() => {
        return;
  ...
  public addEvent(event: Event): Promise<void> {
    ...
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          `INSERT INTO events (id, patient_id, visit_id, event_type, event_timestamp, edited_at, event_metadata) VALUES (?, ?, ?, ?, ?, ?, ?);`,
          [
            id,
            event.patient_id,
            event.visit_id,
      ...
      .then(([results]) => {
        console.log(`[db] Added event with id: "${id}"!`);
      });
  }
  ...
  public addVisit(visit: Visit): Promise<void> {
    ...
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          `INSERT INTO visits (id, patient_id, clinic_id, provider_id, check_in_timestamp, edited_at) VALUES (?, ?, ?, ?, ?, ?);`,
          [
            id,
            visit.patient_id,
            visit.clinic_id,
      ...
      .then(([results]) => {
        console.log(`[db] Added visit with id: "${id}"!`);
      });
  }

  public getLatestPatientEventByType(
    patient_id: string,
    event_type: string,
  ): Promise<string> {
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          'SELECT event_metadata FROM events WHERE patient_id = ? AND event_type = ? ORDER BY event_timestamp DESC LIMIT 1;',
  ...
  public getAllPatientEventsByType(
    patient_id: string,
    event_type: string,
  ): Promise<Event[]> {
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          'SELECT id, patient_id, event_type, event_metadata, edited_at FROM events WHERE patient_id = ? AND event_type = ? ORDER BY event_timestamp DESC;',
          [patient_id, event_type],
        ),
      )
      .then(async ([results]) => {
  ...
  public usersExist(): Promise<boolean> {
    return this.getDatabase()
      .then((db) => db.executeSql(`SELECT * FROM users`))
      .then(([results]) => {
        if (results === undefined) {
          return false;
        }
  ...
  public async getClinics(): Promise<Clinic[]> {
    ...
    return this.getDatabase()
      .then((db) => db.executeSql('SELECT * FROM clinics ORDER BY id DESC;'))
      .then(([results]) => {
        if (results === undefined) {
          return [];
        }
  ...
  public getPatients(): Promise<Patient[]> {
    ...
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          'SELECT id, given_name, surname, date_of_birth, country, hometown, section, serial_number, registered_by_provider_id, sex, phone FROM patients ORDER BY edited_at DESC LIMIT 25;',
        ),
      )
      .then(async ([results]) => {
        if (results === undefined) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const camp = await this.getLatestPatientEventByType(
            id,
            EventTypes.Camp,
          );
          const givenNameContent = await this.languageStringDataById(
            given_name,
          );
          ...
          const sectionContent = await this.languageStringDataById(section);
          console.log(`[db] Patient name: ${given_name}, id: ${id}`);
          patients.push({
            id,
            given_name: givenNameContent,
            surname: surnameContent,
            date_of_birth,
            country: countryContent,
            hometown: hometownContent,
  ...
  public searchPatients(
    ...
    if (!!camp) {
      if (!!queryTerms) {
        queryTerms += ` INTERSECT ${queryBase} WHERE events.event_type = '${
          EventTypes.Camp
        }' AND events.event_metadata LIKE '%${camp.trim()}%'`;
      } else {
        queryTerms += ` WHERE events.event_type = '${
          EventTypes.Camp
        }' AND events.event_metadata LIKE '%${camp.trim()}%'`;
      }
    }
    ...
    if (
      !!minYear &&
      !!maxYear &&
      minYear.toString().length === 4 &&
      maxYear.toString().length === 4
    ) {
      if (!!queryTerms) {
    ...
    console.log('[db] Fetching patients from the db...');
    return this.getDatabase()
      .then((db) => db.executeSql(queryBase + queryTerms))
      .then(async ([results]) => {
        if (results === undefined) {
          return [];
        }
  ...
  public getPatient(patient_id: string): Promise<Patient> {
    ...
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          'SELECT id, given_name, surname, date_of_birth, country, hometown, sex, phone FROM patients WHERE id = ?;',
          [patient_id],
        ),
      )
      .then(async ([results]) => {
        ...
        console.log(`[db] Edited patient with id: "${id}"!`);
  ...
  public getUser(user_id: string): Promise<User> {
    ...
    return this.getDatabase()
      .then((db) =>
        db.executeSql('SELECT id, name, role, email FROM users WHERE id = ?;', [
          user_id,
        ]),
      )
      .then(async ([results]) => {
        ...
        console.log(`[db] Retrieved user with id: "${id}"!`);
        return user;
      });
  }
  ...
  public getVisits(patient_id: string): Promise<Visit[]> {
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          'SELECT v.id, v.patient_id, v.clinic_id, v.provider_id, v.check_in_timestamp, u.name FROM visits as v LEFT JOIN users as u ON v.provider_id=u.id WHERE patient_id = ? AND v.deleted = ? ORDER BY check_in_timestamp DESC;',
          [patient_id, 0],
        ),
      )
      .then(async ([results]) => {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const nameContent =
            name != null
              ? await this.languageStringDataById(name)
              : {id: null, content: {en: 'Unknown'}};
  ...
  public getEvents(visit_id: string): Promise<Event[]> {
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          'SELECT id, patient_id, event_type, event_timestamp, event_metadata FROM events WHERE visit_id = ? ORDER BY event_timestamp DESC;',
          [visit_id],
        ),
      )
      .then(([results]) => {
        ...
        for (let i = 0; i < count; i++) {
          const row = results.rows.item(i);
          const {id, patient_id, event_type, event_timestamp, event_metadata} =
            row;
          events.push({
            id,
            patient_id,
  ...
  public getPatientsVisitedInDateRange(
    startDate: Date,
    endDate: Date,
  ): Promise<any[]> {
    return this.getDatabase()
      .then((db) =>
        db.executeSql(
          'SELECT v.id, v.patient_id, v.clinic_id, v.provider_id, v.check_in_timestamp, p.sex, p.date_of_birth FROM visits as v LEFT JOIN patients as p ON v.patient_id=p.id WHERE v.edited_at BETWEEN ? AND ? ORDER BY check_in_timestamp DESC;',
  ...
  public deleteVisit(visit_id: string, patient_id: string): Promise<Visit[]> {
    ...
    return this.getDatabase()
      .then((db) => {
        db.executeSql(
          'UPDATE visits SET edited_at = ?, deleted = ? WHERE id = ?',
          [date, 1, visit_id],
        );
      })
      .then(() => {
